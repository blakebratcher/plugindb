...
(function () {
  ...
  function pluginCard(p) {
    ...
    const imageHtml = p.image_url
      ? `<div class="card-image"><img src="${escapeHtml(p.image_url)}" alt="${escapeHtml(p.name)}" loading="lazy" onerror="${escapeHtml(onerrorFallback)}"></div>`
      ...
      try {
        ...
        dropdown.innerHTML = data.results.map(r =>
          `<div class="suggest-item" data-slug="${escapeHtml(r.slug)}">
            ${r.image_url ? `<img class="suggest-thumb" src="${escapeHtml(r.image_url)}" alt="" loading="lazy" onerror="this.style.display='none'">` : ''}
  ...
  Views.pluginDetail = async function (slug) {
    ...
    try {
      ...
      const heroHtml = p.image_url
        ? `<div class="pd-hero"><img src="${escapeHtml(p.image_url)}" alt="${escapeHtml(p.name)}" onerror="${escapeHtml(onerrorFallback)}"><div class="pd-hero-overlay"></div></div>`
